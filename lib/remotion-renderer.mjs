...
const assertLocalRef = (value) => {
  if (typeof value !== 'string' || value.length === 0) {
    throw new TypeError('Render media reference must be a non-empty string');
  }
  if (value.startsWith('data:')) {
    if (Buffer.byteLength(value, 'utf8') > MAX_DATA_URL_BYTES) {
      throw new TypeError('In-memory render media exceeds the bounded data URL limit');
    }
    return;
  }
  if (value.length > 4096) throw new TypeError('Render media reference must be bounded');
...
export async function renderBrightProfile({
  ...
  const safeInputProps = await materializeLocalMedia(inputProps, assetRoot ?? publicDir);
