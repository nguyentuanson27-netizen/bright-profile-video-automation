...
const sourceObservationCompleteness = (source) => [
  source.title,
  source.author,
  source.publishedAt,
  source.excerpt,
  source.sourceRelationship,
  source.publisher && source.publisher !== publisherDomain(source.canonicalUrl) ? source.publisher : undefined,
  source.sourceType && source.sourceType !== 'other' ? source.sourceType : undefined,
].filter(Boolean).length;

const publishedAtOrder = (left, right) => {
...
const compareSourceObservations = (left, right) => sourceObservationCompleteness(right) - sourceObservationCompleteness(left)
  || publishedAtOrder(left, right)
  || (SOURCE_PRIOR[right.sourceType] ?? SOURCE_PRIOR.other) - (SOURCE_PRIOR[left.sourceType] ?? SOURCE_PRIOR.other)
  || (SOURCE_RELATIONSHIP_PRIOR[right.sourceRelationship] ?? 0) - (SOURCE_RELATIONSHIP_PRIOR[left.sourceRelationship] ?? 0)
  || (right.excerpt?.length ?? 0) - (left.excerpt?.length ?? 0)
  || left.url.localeCompare(right.url)
  || JSON.stringify(left).localeCompare(JSON.stringify(right));

const mergeSource = (left, right) => {
  if (sourceKey(left) !== sourceKey(right)) return left;
  return compareSourceObservations(left, right) <= 0 ? {...left} : {...right};
};
