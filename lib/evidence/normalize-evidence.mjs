...
const preferEarlierPublishedAt = (left, right) => {
  if (!left) return right;
  if (!right) return left;
  if (left === right) return left;
...
const mergeSource = (left, right) => {
  ...
  for (const key of ['title', 'author', 'excerpt']) {
    const value = preferDeterministicText(left[key], right[key]);
    if (value) merged[key] = value;
    else delete merged[key];
  }
  const publishedAt = preferEarlierPublishedAt(left.publishedAt, right.publishedAt);
  if (publishedAt) merged.publishedAt = publishedAt;
  else delete merged.publishedAt;
  ...
  else delete merged.sourceRelationship;
  return merged;
};
