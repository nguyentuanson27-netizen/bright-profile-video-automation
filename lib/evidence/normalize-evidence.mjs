...
import {classifyInvalidItems} from './schema-validator.mjs';
...
const selectEvidence = (evidence, conflicts, maxEvidence) => {
  ...
  for (const unit of conflictUnits) {
    if (unit.items.length > remaining) continue;
    for (const item of unit.items) selectedIds.add(item.id);
    remaining -= unit.items.length;
  }
...
export function normalizeEvidence(input) {
  ...
  for (const candidate of normalized) {
    ...
    if (existingIndex !== undefined) {
      ...
      for (const key of [...keys, exact[existingIndex].fingerprint]) exactByKey.set(key, existingIndex);
      continue;
    }
