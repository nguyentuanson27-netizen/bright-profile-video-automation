...
const claimNumericMarkers = (evidence) => {
  const rawNumbers = evidence.claimNormalized.match(/\d+(?:[.,]\d+)*/g) ?? [];
  const claimTokens = tokensFor(evidence.claimNormalized);
  const hasNamedMonth = [...MONTH_TOKENS].some((token) => claimTokens.has(token));
  const isoClaimDate = String(evidence.claimDate ?? '').slice(0, 10);
  const explicitlyMentionsIsoDate = Boolean(isoClaimDate && evidence.claimNormalized.includes(isoClaimDate));
  const dateParts = hasNamedMonth || explicitlyMentionsIsoDate ? [...claimDateParts(evidence)] : [];
  const markers = [];
  for (const raw of rawNumbers) {
    const comparableRaw = /^\d+$/.test(raw) ? String(Number(raw)) : raw;
    const dateIndex = dateParts.indexOf(comparableRaw);
    if (dateIndex >= 0) {
      dateParts.splice(dateIndex, 1);
      continue;
    }
    const numericRaw = /^\d+$/.test(raw) ? Number(raw) : NaN;
    if (hasNamedMonth && Number.isInteger(numericRaw) && numericRaw >= 1900 && numericRaw <= 2100) continue;
