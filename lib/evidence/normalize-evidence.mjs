...
const claimWithoutDeclaredDate = (evidence) => {
  ...
  if (!match) return evidence.claimNormalized;
  const [, year, month, day] = match;
  let text = evidence.claimNormalized.replaceAll(`${year}-${month}-${day}`, ' ');
  const monthNames = (MONTH_VARIANTS[month] ?? []).filter((token) => /\p{L}/u.test(token));
  if (!monthNames.length) return text;

  const monthPattern = monthNames.map(escapePattern).join('|');
  const dayPattern = [...new Set([day, String(Number(day))])].map(escapePattern).join('|');
  const patterns = [
    new RegExp(`\\b(?:${monthPattern})\\s+(?:${dayPattern})(?:st|nd|rd|th)?\\s+${escapePattern(year)}\\b`, 'gu'),
    new RegExp(`\\b(?:${dayPattern})(?:st|nd|rd|th)?\\s+(?:${monthPattern})\\s+${escapePattern(year)}\\b`, 'gu'),
    new RegExp(`\\b(?:${monthPattern})\\s+${escapePattern(year)}\\b`, 'gu'),
  ];
