...
When duplicate evidence points to the same canonical URL, the normalizer does **not** synthesize a source object by taking conflicting fields from different observations. It deterministically selects one complete normalized source observation and emits that observation intact, preserving a traceable title/author/date/excerpt/provenance combination that actually occurred in the caller input. Selection uses one fixed total order: explicit provenance completeness descending; then, for equally complete observations, earlier `publishedAt` (missing dates sort after present dates); then source-type prior, source-relationship prior, excerpt length, raw URL, and full-object lexical tie-breaks. This keeps sparse-vs-rich selection useful, keeps equal-completeness freshness conservative, and makes two-way or multi-way same-canonical merging permutation-stable without fabricating hybrid provenance. Spec amendment `002-review-4914663081.md` makes this policy part of canonical Spec 001 and narrowly supersedes the original Step 6 field-wise metadata/excerpt preference when it would break same-source provenance coherence.
